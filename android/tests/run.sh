...
"$JAVA_HOME/bin/javac" -encoding UTF-8 -d "$OUT" "$SRC/MdictFile.java" "$SRC/HtmlText.java" "$SRC/Lzo.java" "$SRC/Ripemd128.java" "$SRC/Fsrs.java" "$SRC/Deinflect.java" "$SRC/ZipSource.java" "$SRC/BookParser.java" "$SRC/MarkupFix.java" "$SRC/Yomitan.java" "$HERE/CoreTests.java"
